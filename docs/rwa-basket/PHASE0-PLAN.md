...
**部署状态**（2026-09-18 晚）：✅ Render `dep-damgucbm8hqs73d420ag` live（提交 `12bdf44`）；生产三库迁移至 36 号；
线上实证——目录 7,940 资产全量入库（8m47s，限速 27req/min）、行情 7,938/7,940、评分 7,938、DeepSeek AI 实弹 2/2、
`/v1/rwa/*` 全端点返回真实数据（gold 实时价+发行商+评分+AI 摘要）。Worker 已启用：sync（每日/每小时）、analysis（6h/每日）、basket-ops（6h/每日）。
Basket 开关保持默认关闭；执行适配器=manual（不接真实资金）。已知事项：队列 dedupKey 一旦 dead 会阻塞当日重入队（次日/下一小时自动恢复）。
