...
const STATUS_STYLES: Record<string,string> = { pending:'bg-amber-500/20 text-amber-600', distributed:'bg-mint/20 text-mint', failed:'bg-red-500/20 text-red-600' };
...
export default function YieldsPage() {
  ...
  return (<AdminLayout><div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <Card className="glass-strong"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">待发放</p><p className="text-2xl font-bold mt-1 text-amber-600">{items.filter(i=>i.status==='pending').length}</p></div><Clock className="w-10 h-10 text-amber-600/50" /></div></CardContent></Card>
      <Card className="glass-strong"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">发放总额(USD)</p><p className="text-2xl font-bold mt-1 text-sky-600">{items.filter(i=>i.status==='distributed').reduce((s,i)=>s+parseFloat(i.amount.replace(/[$,]/g,'')),0).toLocaleString()}</p></div><DollarSign className="w-10 h-10 text-sky-600/50" /></div></CardContent></Card>
    </div>
    <Card className="glass-strong"><CardContent className="p-6"><form onSubmit={(e)=>{e.preventDefault();setCurrentPage(1);fetchData();}} className="flex flex-col sm:flex-row gap-4">
      ...
      <select className="px-3 py-2 rounded-lg bg-ink/[0.04] border border-ink/[0.07]" value={typeFilter} onChange={(e)=>setTypeFilter(e.target.value)}><option value="">全部类型</option><option value="staking">质押收益</option><option value="dividend">分红</option><option value="interest">利息</option><option value="capital_gain">资本利得</option></select>
      <select className="px-3 py-2 rounded-lg bg-ink/[0.04] border border-ink/[0.07]" value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)}><option value="">全部状态</option><option value="pending">待发放</option><option value="distributed">已发放</option><option value="failed">失败</option></select>
    ...
    <Card className="glass-strong"><CardHeader className="flex flex-row items-center justify-between"><CardTitle>收益列表 (共 {totalCount} 条)</CardTitle><div className="flex items-center gap-2"><span className="text-sm text-muted-foreground">每页</span><select className="px-2 py-1 rounded bg-ink/[0.04] border border-ink/[0.07]" value={pageSize} onChange={(e)=>{setPageSize(Number(e.target.value));setCurrentPage(1);}}><option value={10}>10</option><option value={20}>20</option><option value={50}>50</option></select></div></CardHeader>
      <CardContent className="p-0">{loading?(<div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-mint" /></div>):items.length===0?(<div className="flex flex-col items-center justify-center p-12 text-muted-foreground"><TrendingUp className="w-12 h-12 mb-4 opacity-50" /><p>暂无收益记录</p></div>):(
        <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>用户</TableHead><TableHead>资产</TableHead><TableHead className="w-24">类型</TableHead><TableHead className="w-28">金额</TableHead><TableHead className="w-24">链</TableHead><TableHead className="w-24">状态</TableHead><TableHead className="w-28">发放日期</TableHead><TableHead className="w-24">操作</TableHead></TableRow></TableHeader><TableBody>
          {items.map((y)=>(<TableRow key={y.id} className="hover:bg-ink/[0.05]">
            ...
            <TableCell className="font-mono tabular-nums text-sky-600">{y.amount}</TableCell>
