...
const STATUS_STYLES: Record<string, string> = { pending: 'bg-amber-500/20 text-amber-600', processing: 'bg-blue-500/20 text-blue-600', completed: 'bg-mint/20 text-mint', failed: 'bg-red-500/20 text-red-600' };
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card className="glass-strong"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">待结算</p><p className="text-2xl font-bold mt-1 text-amber-600">{items.filter(i => i.status === 'pending' || i.status === 'processing').length}</p></div><Clock className="w-10 h-10 text-amber-600/50" /></div></CardContent></Card>
          <Card className="glass-strong"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">结算总额(USD)</p><p className="text-2xl font-bold mt-1 text-sky-600">{items.filter(i => i.status === 'completed').reduce((s, i) => s + parseFloat(i.amount.replace(/[$,]/g, '')), 0).toLocaleString()}</p></div><DollarSign className="w-10 h-10 text-sky-600/50" /></div></CardContent></Card>
        </div>
        <Card className="glass-strong">
          <CardContent className="p-6">
            <form onSubmit={(e) => { e.preventDefault(); setCurrentPage(1); fetchData(); }} className="flex flex-col sm:flex-row gap-4">
              ...
              <select className="px-3 py-2 rounded-lg bg-ink/[0.04] border border-ink/[0.07]" value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}><option value="">全部类型</option><option value="trade">交易结算</option><option value="yield">收益结算</option><option value="redemption">赎回结算</option><option value="fee">手续费结算</option></select>
              <select className="px-3 py-2 rounded-lg bg-ink/[0.04] border border-ink/[0.07]" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">全部状态</option><option value="pending">待结算</option><option value="processing">结算中</option><option value="completed">已结算</option><option value="failed">失败</option></select>
          ...
          <CardContent className="p-0">
            {loading ? (<div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-mint" /></div>) : items.length === 0 ? (<div className="flex flex-col items-center justify-center p-12 text-muted-foreground"><Scale className="w-12 h-12 mb-4 opacity-50" /><p>暂无结算记录</p></div>) : (
              <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>用户</TableHead><TableHead className="w-24">资产</TableHead><TableHead className="w-28">金额</TableHead><TableHead className="w-24">类型</TableHead><TableHead className="w-24">链</TableHead><TableHead className="w-24">状态</TableHead><TableHead className="w-28">时间</TableHead><TableHead className="w-24">操作</TableHead></TableRow></TableHeader><TableBody>
                {items.map((s) => (
                  <TableRow key={s.id} className="hover:bg-ink/[0.05]">
                    ...
                    <TableCell className="font-mono tabular-nums text-sky-600">{s.amount}</TableCell>
                    <TableCell><Badge variant="outline" className="bg-blue-500/20 text-blue-600">{TYPE_LABELS[s.settlementType]}</Badge></TableCell>
