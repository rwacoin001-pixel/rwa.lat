...
const STATUS_STYLES: Record<string,string> = { idle:'bg-gray-500/20 text-slate-500', running:'bg-blue-500/20 text-blue-600', completed:'bg-mint/20 text-mint', failed:'bg-red-500/20 text-red-600' };
...
export default function AiOpsPage() {
  ...
  return (<AdminLayout><div className="space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <Card className="glass-strong"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">运行中</p><p className="text-2xl font-bold mt-1 text-blue-600">{items.filter(i=>i.status==='running').length}</p></div><Cpu className="w-10 h-10 text-blue-600/50" /></div></CardContent></Card>
      ...
      <Card className="glass-strong"><CardContent className="p-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">总成本(USD)</p><p className="text-2xl font-bold mt-1 text-sky-600">{items.reduce((s,i)=>s+parseFloat(i.costUsd.replace('$','')),0).toFixed(2)}</p></div><DollarSign className="w-10 h-10 text-sky-600/50" /></div></CardContent></Card>
    </div>
    <Card className="glass-strong"><CardContent className="p-6"><form onSubmit={(e)=>{e.preventDefault();setCurrentPage(1);fetchData();}} className="flex flex-col sm:flex-row gap-4">
      ...
      <select className="px-3 py-2 rounded-lg bg-ink/[0.04] border border-ink/[0.07]" value={typeFilter} onChange={(e)=>setTypeFilter(e.target.value)}><option value="">全部类型</option><option value="content_generation">内容生成</option><option value="risk_assessment">风控评估</option><option value="user_segmentation">用户分群</option><option value="report_generation">报告生成</option><option value="anomaly_detection">异常检测</option></select>
      <select className="px-3 py-2 rounded-lg bg-ink/[0.04] border border-ink/[0.07]" value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)}><option value="">全部状态</option><option value="idle">空闲</option><option value="running">运行中</option><option value="completed">已完成</option><option value="failed">失败</option></select>
      <Button type="submit" className="flex items-center gap-2"><Filter className="w-4 h-4" />筛选</Button>
    </form></CardContent></Card>
    <Card className="glass-strong"><CardHeader className="flex flex-row items-center justify-between"><CardTitle>AI任务列表 (共 {totalCount} 条)</CardTitle><div className="flex items-center gap-2"><span className="text-sm text-muted-foreground">每页</span><select className="px-2 py-1 rounded bg-ink/[0.04] border border-ink/[0.07]" value={pageSize} onChange={(e)=>{setPageSize(Number(e.target.value));setCurrentPage(1);}}><option value={10}>10</option><option value={20}>20</option><option value={50}>50</option></select></div></CardHeader>
      <CardContent className="p-0">{loading?(<div className="flex items-center justify-center h-64"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-mint" /></div>):items.length===0?(<div className="flex flex-col items-center justify-center p-12 text-muted-foreground"><Brain className="w-12 h-12 mb-4 opacity-50" /><p>暂无AI任务</p></div>):(
        <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>任务名称</TableHead><TableHead className="w-28">类型</TableHead><TableHead className="w-28">模型</TableHead><TableHead className="w-24">Tokens</TableHead><TableHead className="w-20">成本</TableHead><TableHead className="w-24">状态</TableHead><TableHead className="w-28">时间</TableHead><TableHead className="w-24">操作</TableHead></TableRow></TableHeader><TableBody>
          {items.map((t)=>(<TableRow key={t.id} className="hover:bg-ink/[0.05]">
            ...
            <TableCell className="font-mono tabular-nums text-sky-600">{t.costUsd}</TableCell>
